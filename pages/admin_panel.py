        ...
        try:
            ...
            if pdfs:
                for pdf in pdfs:
                    ...
                    cols[2].markdown(f"📅 {pd.to_datetime(pdf['upload_date']).strftime('%Y-%m-%d')}")  # Fixed closing }
                    cols[3].markdown(f"📦 {float(pdf.get('size', Decimal(0))) / 1e6:.1f}MB")  # Fixed parentheses
